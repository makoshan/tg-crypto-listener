...
→ 需要量化宏观背景来解释价格波动
→ 搜索证据不足以判定宏观影响时
→ **多重宏观风险叠加时**（如：政府关门 + 地缘冲突 + 美股高位回调）
→ **宏观不确定性主导市场时**（需要客观数据评估风险程度）
...
**宏观多因素分析原则**：
- 当消息提到"多重宏观干扰"时，优先获取 VIX（恐慌程度）和 DXY（美元避险）
- 政府关门/债务危机 → 获取 `VIX` + `DXY`
- 战争/地缘冲突 → 获取 `VIX` + `DXY`（避险资金流向）
- 美股回调风险 → 获取 `VIX`（波动率）验证市场恐慌程度
- 降息预期 → 获取 `FED_FUNDS`（当前利率）判断降息空间
- **多重负面因素叠加** → 建议获取 2-3 个互补指标量化风险
...
**宏观-加密市场传导机制**：
1. **流动性收紧路径**（利空）：
   - 美联储加息/缩表 → 美元走强(DXY↑) → 风险资产抛售 → 加密市场承压
   - 银行信用收紧 → 流动性枯竭 → 杠杆平仓 → 加密市场暴跌

2. **避险情绪路径**（利空）：
   - 战争/地缘冲突 → VIX 飙升 → 风险偏好下降 → 资金流向黄金/美元 → 加密市场抛售
   - 美股暴跌 → 恐慌传染 → 加密市场联动下跌（滞后 2-6h，跌幅放大 1.5-2x）

3. **政策不确定性路径**（利空）：
   - 政府关门/债务危机 → 政策真空 → 市场观望 → 交易量萎缩 → 流动性下降
   - 监管收紧预期 → 合规成本上升 → 机构资金撤离

4. **宽松预期路径**（利好）：
   - 降息预期/实施 → 流动性改善 → 风险资产反弹 → 加密市场上涨
   - 经济衰退担忧 → 宽松政策预期 → 对冲通胀需求 → BTC 作为"数字黄金"受益

**多重宏观干扰因素叠加效应判断**：

当消息中同时出现多个宏观负面因素时（如：政府关门 + 中美紧张 + 战争 + 美股回调），需要：

1. **量化每个因素的当前状态**（基于宏观数据）：
   - VIX > 30 → 市场高度恐慌，风险资产面临抛压
   - VIX 20-30 → 中度紧张，观望情绪浓厚
   - VIX < 20 → 市场相对平静，恐慌有限

   - DXY > 105 → 美元强势避险，压制加密市场
   - DXY 100-105 → 美元中性，影响有限
   - DXY < 100 → 美元走弱，利好风险资产

2. **评估因素间的相互强化 vs 对冲**：
   - **强化效应**：政府关门 + 债务危机 → 双重政策不确定性 → 加重避险情绪
   - **强化效应**：美股暴跌 + VIX 飙升 → 恐慌传染 → 加密市场联动下跌
   - **对冲效应**：经济衰退预期 + 降息预期 → 短期利空但中期利好 → 波动加剧

3. **判断宏观因素的时间尺度**：
   - **短期冲击**（1-2 周）：政府关门、VIX 飙升 → 暂时性抛压，不改变长期趋势
   - **中期压制**（1-3 月）：加息周期、地缘冲突 → 持续压制风险偏好
   - **长期利好**（3-12 月）：降息周期、流动性宽松 → 驱动牛市

4. **置信度调整原则**：
   - **单一宏观因素** + 数据验证 → 调整 ±0.10
   - **2-3 个宏观因素叠加** + 数据验证 → 调整 ±0.15 to ±0.25
   - **多重负面因素 + VIX>30 + DXY>105** → 大幅降低置信度 -0.30 to -0.50，action 倾向 "sell" 或 "observe"
   - **唯一确定利好（降息）vs 多重干扰** → 降低买入信号置信度 -0.20，延长观望期

**宏观背景下的操作建议调整**：
- 如果初步判断为 "buy" 但宏观多重负面叠加 → 改为 "observe"，等待宏观不确定性消除
- 如果初步判断为 "sell" 且宏观数据验证（VIX 高、DXY 强）→ 提升置信度，强化卖出信号
- 如果宏观因素时间尺度为"短期冲击"→ 在 notes 中说明"短期波动，不改变中长期趋势"
...
- **如果涉及宏观因素，必须在 notes 中说明**：
  - "宏观背景: [具体说明哪些宏观因素及其影响]"
  - "宏观数据: VIX={vix_value}, DXY={dxy_value} [如果获取到数据]"
  - "宏观影响: [利空/利好/中性]，[短期/中期/长期]"
  - "多重干扰叠加: [列出所有负面因素及叠加效应]" （如适用）
  - "宏观调整: 置信度 [+/-X.XX]，action [调整说明]" （如果因宏观因素调整）
...
- **如果多重宏观负面因素叠加**:
  - 必须添加 risk_flag: "macro_multiple_headwinds"
  - 如果 VIX > 30 或 DXY > 105，添加 risk_flag: "high_macro_risk"
  - 在 summary 中说明 "宏观多重干扰，市场脆弱"
