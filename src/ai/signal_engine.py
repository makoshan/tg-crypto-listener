...
def build_signal_prompt(payload: EventPayload) -> str:
    ...
    return (
        ...
        "notes(简短中文描述，可为空)。"
        "字段规范：summary 最多 50 个汉字且不得包含换行或多余空格；action 为 observe 时也要说明原因；notes 用一句话交代行动依据或需关注点，不得返回“暂无”或重复 summary；confidence 与 strength 需对应：<0.3 为 low、0.3-0.69 为 medium、≥0.7 为 high。"
        "规则：1) 金额或影响力不足(如<3000美元、信息模糊) → action=observe, confidence≤0.3，risk_flags 至少包含 confidence_low 或 data_incomplete。"
        "2) liquidations/whale 需对比 historical_reference 判定是否显著，在 notes 中说明。"
        "3) 若存在宏观/政策/名人言论等潜在驱动，请指出潜在交易机会或关注项。"
        "4) 信息缺失或不确定时，在 risk_flags 加 confidence_low 或 data_incomplete。"
        "5) 输出必须为简体中文，数值保留两位小数，字符串去除首尾空格。"
        "输入 JSON："
        f"{context_json}"
    )
