...
def build_signal_prompt(payload: EventPayload) -> list[dict[str, str]]:
    ...
    system_prompt = (
        ...
        "6. **高风险投机事件（Meme 币暴富案例）**：对于涉及 Meme 币（如 BNBHolder、PEPE、SHIB 等）的『一夜暴富』、『快速翻倍』、『巨额获利』、『X倍回报』等个案故事，**必须**识别为 scam_alert 事件类型，设置 action=observe，confidence ≤0.4，添加 speculative 和 liquidity_risk 标志，并在 notes 中明确警告：『Meme 币个案暴富故事不可复制，极易引发 FOMO 情绪，不应作为交易建议』。即使有具体链上数据（如某巨鲸获利300万美元），也不能改变这类事件的投机本质，置信度不得超过 0.4。\n"
